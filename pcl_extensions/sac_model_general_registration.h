#ifndef PCL_SAMPLE_CONSENSUS_MODEL_GENERAL_REGISTRATION_H_
#define PCL_SAMPLE_CONSENSUS_MODEL_GENERAL_REGISTRATION_H_
...
namespace pcl
{
  ...
  class SampleConsensusModelGeneralRegistration : public SampleConsensusModel<PointT>
  {
  public:
    using SampleConsensusModel<PointT>::input_;
    using SampleConsensusModel<PointT>::indices_;
    using SampleConsensusModel<PointT>::shuffled_indices_;
    using SampleConsensusModel<PointT>::error_sqr_dists_;
    using SampleConsensusModel<PointT>::rnd;
    using SampleConsensusModel<PointT>::samples_radius_;
    using SampleConsensusModel<PointT>::getSampleSize;
    ...
    typedef boost::shared_ptr<SampleConsensusModelGeneralRegistration> Ptr;

    /** \brief Constructor for base SampleConsensusModelRegistration.
      * \param[in] cloud the input point cloud dataset
      * \param[in] random if true set the random seed to the current time, else set to 12345 (default: false)
      */
    SampleConsensusModelGeneralRegistration (const PointCloudConstPtr &cloud,
        bool random = false)
    ...
    SampleConsensusModelGeneralRegistration (const PointCloudConstPtr &cloud,
      ...
      , sample_dist_thresh_ (0)
    {
      indices_src_.reset(new std::vector<int> (indices));
    ...
    virtual ~SampleConsensusModelGeneralRegistration () {}
    ...
    /** \brief Get a set of random data samples and return them as indices.
    ...
    getSamples (int &iterations, std::vector<int> &samples)
    {
      ...
      const size_t c = indices_tgt_->size();
  ...
  protected:
    ...
    estimateRigidTransformationSVD (const pcl::PointCloud<PointT> &cloud_src,
                                    ...
                                    const std::vector<int> &correspondence_indices_,
    ...
    computeOriginalIndexMapping ()
    {
      if (!indices_tgt_ || !indices_src_ || indices_src_->empty () || indices_src_->size () != indices_tgt_->size ())
        return;
      samples_radius_ = 0;
    }
    ...
    boost::shared_ptr <std::vector<int> > indices_src_;
...
#include <sac_model_general_registration.hpp>
